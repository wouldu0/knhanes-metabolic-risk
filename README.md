...
분석 대상자의 대사증후군 구성요소를 살펴본 결과, 복부비만과 고중성지방혈증이 상대적으로 높은 비율을 보였습니다.

![대사증후군 5대 구성요소 이상 비율 (비가중)](results/chart5_ms_components.png)
...
아래는 운동 그룹별 대사증후군 유병률을 표본 그대로(비가중) 집계한 값입니다 — 다른 요인을 보정하지 않은 단순 비교이며, 나이·성별·흡연·음주를 보정한 조정 연관성은 위 Forest Plot의 OR을 기준으로 봐야 합니다.

![운동 그룹별 대사증후군 유병률 (비가중)](results/chart2_exercise_group_prevalence.png)
...
> 기존 프로젝트 결과보다 일부 성능은 낮아졌지만, 데이터 누수를 방지하고 test set을 최종 평가에만 사용해 더 신뢰할 수 있는 평가 구조로 다시 측정한 결과입니다. 자세한 배경은 아래 "🔧 ML 파이프라인 리팩터링" 참고. (참고: KNHANES 표본가중치를 반영한 Sample-weighted ROC-AUC는 0.7281로 비슷한 수준입니다.)
...
- 액상형 전자담배는 비교 가능한 과거사용 문항이 없어 현재사용 여부만 흡연 상태 판정에 반영함
